/**
 * TIFF to JPG Converter Module
 */

let selectedFiles = [];

export async function init() {
    console.log('🖼️ TIFF to JPG converter initialized');
    ...
    optionsContent.innerHTML = `
        <div class="option" style="background: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.3); padding: 15px; border-radius: 8px; margin-bottom: 20px;">
            <strong style="color: #ef4444; display: block; margin-bottom: 8px;">⚠️ Browser Compatibility Warning</strong>
            <p style="color: var(--text-secondary); font-size: 13px; margin: 0;">TIFF format is not natively supported by most browsers. Conversion may not work for all TIFF variants. For best results, use Chrome or Edge with basic TIFF files.</p>
        </div>
        <div class="option">
    ...
    function handleFiles(files) {
        ...
        for (let file of files) {
            ...
            if (!isTIFF) {
                alert('Please select TIFF files only.');
                continue;
            }
    ...
    convertBtn.addEventListener('click', async function() {
        ...
        for (let i = 0; i < selectedFiles.length; i++) {
            ...
            try {
                const blob = await convertTIFFtoJPG(file, quality, resizeValue);
                const filename = file.name.replace(/\.(tiff|tif)$/i, '.jpg');
                convertedFiles.push({ blob, filename });
            } catch (error) {
                console.error('Conversion error:', error);
                alert(`Failed to convert ${file.name}. TIFF format may not be fully supported in your browser.`);
            }
        }
    ...
    async function convertTIFFtoJPG(file, quality, resizeValue) {
        return new Promise((resolve, reject) => {
            ...
            reader.onload = function(e) {
                ...
                img.onerror = () => reject(new Error('Failed to load TIFF'));
