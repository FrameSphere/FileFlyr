/**
 * AVIF to JPG Converter Module
 */

let selectedFiles = [];

export async function init() {
    console.log('🖼️ AVIF to JPG converter initialized');
    ...
    function handleFiles(files) {
        ...
        for (let file of files) {
            if (!file.type.startsWith('image/avif')) {
                alert('Please select AVIF files only.');
                continue;
            }
    ...
    convertBtn.addEventListener('click', async function() {
        ...
        for (let i = 0; i < selectedFiles.length; i++) {
            ...
            try {
                const blob = await convertImageToJPG(file, quality, resizeValue);
                const filename = file.name.replace(/\.avif$/i, '.jpg');
    ...
    async function convertImageToJPG(file, quality, resizeValue) {
