...
export async function init() {
    ...
    convertBtn.addEventListener('click', async function() {
        ...
        for (let i = 0; i < selectedFiles.length; i++) {
            ...
            try {
                const images = await convertPDFtoJPG(file, quality, scale);
                images.forEach((blob, pageNum) => {
                    const filename = file.name.replace('.pdf', `-page${pageNum + 1}.jpg`);
    ...
    async function convertPDFtoJPG(file, quality, scale) {
