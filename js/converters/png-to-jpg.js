/**
 * PNG to JPG Converter Module
 */

let selectedFiles = [];

export async function init() {
    console.log('🖼️ PNG to JPG converter initialized');
    ...
    function handleFiles(files) {
        ...
        for (let file of files) {
            if (!file.type.startsWith('image/png')) {
                window.showError?.('Please select PNG files only.') || alert('Please select PNG files only.');
                continue;
            }

            if (!window.checkFileSize?.(file, 50)) continue;
            ...
            reader.onload = function(e) {
                ...
                preview.innerHTML = `
                    ...
                    <p>${file.name}</p>
                    <p style="color: var(--text-tertiary); font-size: 11px;">${window.formatFileSize?.(file.size) || file.size}</p>
                `;
    ...
    convertBtn.addEventListener('click', async function() {
        ...
        for (let i = 0; i < selectedFiles.length; i++) {
            ...
            try {
                const blob = await convertPNGtoJPG(file, quality, bgColor, resizeValue);
    ...
    // Convert function
    async function convertPNGtoJPG(file, quality, bgColor, resizeValue) {
